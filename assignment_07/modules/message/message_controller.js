...
const sendMessage = async (req, res) => {
  ...
  const findUser = await userModel.findById(receiverId).select('name email messages');

  if (findUser) {
    const sendedMessage = await messageModel.create({ messageBody, receiverId, senderId });

    // https://stackoverflow.com/a/33049923/16107539
    findUser.messages.push(sendedMessage.id);
