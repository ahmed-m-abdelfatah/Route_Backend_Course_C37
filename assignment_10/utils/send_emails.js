...
const sendConfirmationEmailQL = (originUrl, user, expiration = 1 * 60) => {
  const token = jwt.sign({ id: user.id }, process.env.JWT_EMAIL_SECRET_KEY, { expiresIn: expiration });
  const confirmUrl = `${originUrl}/auth/confirmEmail/${token}`;
  const refreshToken = `${originUrl}/auth/resendToken/${user._id}`;
